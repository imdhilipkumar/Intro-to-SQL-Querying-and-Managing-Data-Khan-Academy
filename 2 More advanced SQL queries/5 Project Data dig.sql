...
*/
SELECT 
  AVG(Worldwide) AS avg_worldwide,
  MAX(Worldwide) AS max_worldwide,
  MIN(Worldwide) AS min_worldwide,
  
  AVG(Domestic) AS avg_domestic,
  MAX(Domestic) AS max_domestic,
  MIN(Domestic) AS min_domestic,
  
  AVG(Overseas) AS avg_overseas,
  MAX(Overseas) AS max_overseas,
  MIN(Overseas) AS min_overseas,
  
  AVG(DomesticPct) AS avg_domestic_pct,
  MAX(DomesticPct) AS max_domestic_pct,
  MIN(DomesticPct) AS min_domestic_pct,
  
  AVG(OverseasPct) AS avg_overseas_pct,
  MAX(OverseasPct) AS max_overseas_pct,
  MIN(OverseasPct) AS min_overseas_pct
...
FROM topmovies
GROUP BY Year
HAVING AVG(Worldwide) > 800;

/*
What ways are there to group the data values that don’t exist yet (using CASE)?
*/
-- 1. Group by earnings tier
SELECT 
  CASE 
    WHEN Worldwide >= 2000 THEN 'Blockbuster (>= $2B)'
    WHEN Worldwide >= 1000 THEN 'Hit ($1B - $2B)'
    WHEN Worldwide >= 750 THEN 'Successful ($750M - $1B)'
    ELSE 'Moderate (< $750M)'
  END AS earnings_group,
  COUNT(*) AS movie_count,
  AVG(Worldwide) AS avg_earnings
FROM topmovies
GROUP BY earnings_group;

-- 2. Group by release decade
SELECT 
  CASE 
...
-- 3. Group by revenue dominance (Domestic vs Overseas)
SELECT 
  CASE 
    WHEN Domestic > Overseas THEN 'Domestic Majority'
    WHEN Overseas > Domestic THEN 'Overseas Majority'
    ELSE 'Equal Split'
  END AS revenue_dominance,
  COUNT(*) AS movie_count,
  AVG(Worldwide) AS avg_earnings
FROM topmovies
GROUP BY revenue_dominance;

-- 4. Group by franchise (approximate via keywords)
SELECT 
  CASE 
    WHEN Title LIKE '%Harry Potter%' THEN 'Harry Potter'
    WHEN Title LIKE '%Avengers%' OR Title LIKE '%Iron Man%' OR Title LIKE '%Captain America%' THEN 'Marvel'
    WHEN Title LIKE '%Transformers%' THEN 'Transformers'
    WHEN Title LIKE '%Twilight%' THEN 'Twilight'
    WHEN Title LIKE '%Star Wars%' THEN 'Star Wars'
...
-- 5. Group by success region (DomesticPct vs OverseasPct)
SELECT 
  CASE 
    WHEN DomesticPct >= 0.5 THEN 'Mostly Domestic'
    WHEN OverseasPct >= 0.5 THEN 'Mostly Overseas'
    ELSE 'Even Split'
  END AS success_region,
  COUNT(*) AS movie_count,
  AVG(Worldwide) AS avg_earnings
FROM topmovies
GROUP BY success_region;

/* What interesting ways are there to filter the data (using AND/OR)?
*/

-- 1. Recent blockbusters (after 2010, over $1B)
SELECT * 
FROM topmovies
WHERE Year > 2010 AND Worldwide > 1000;

-- 2. Surprising performers (low domestic %, high worldwide)
SELECT * 
FROM topmovies
WHERE DomesticPct < 0.3 AND Worldwide > 900;

-- 3. Movies by Disney or Warner Bros
SELECT * 
FROM topmovies
WHERE Studio = 'BV' OR Studio = 'WB';

-- 4. Old or extremely high-grossing movies
SELECT * 
FROM topmovies
WHERE Year < 2000 OR Worldwide > 1500;

-- 5. High overseas dependency
SELECT * 
FROM topmovies
WHERE Worldwide > 800 AND OverseasPct > 0.70;

-- 6. Balanced revenue distribution (domestic ≈ overseas)
SELECT * 
FROM topmovies
WHERE ABS(DomesticPct - OverseasPct) <= 0.05;

-- 7. Exclude Ice Age, Twilight, Transformers
SELECT * 
FROM topmovies
WHERE Title NOT LIKE '%Ice Age%' 
  AND Title NOT LIKE '%Twilight%' 
  AND Title NOT LIKE '%Transformers%';

-- 8. Movies from Sony or Fox (2005–2015)
SELECT * 
FROM topmovies
WHERE Year BETWEEN 2005 AND 2015 
  AND (Studio = 'Sony' OR Studio = 'Fox');

-- 9. Strong U.S. performers (DomesticPct > 50%)
SELECT * 
FROM topmovies
WHERE Worldwide > 500 AND DomesticPct > 0.5;

-- 10. Recent Disney or Universal movies
SELECT * 
FROM topmovies
WHERE Year > 2010 AND (Studio = 'BV' OR Studio = 'Uni.');
